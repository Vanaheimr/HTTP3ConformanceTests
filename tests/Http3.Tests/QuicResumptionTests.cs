    ...
    public void ClientResumesAgainstOwnServer_OverDatagrams_NoCertificate()
    {
        ...
        using (var server = new QuicServerConnection(cert, resumptionCache: cache))
        {
            ...
            for (int r = 0; r < 40 && !client.HandshakeConfirmed; r++)
                Pump(client, server);
            Assert.True(client.HandshakeConfirmed, "Handshake (Verbindung 1) muss abschließen.");
            ...
            for (int r = 0; r < 6 && client.NewSessionTickets.Count == 0; r++)
                Pump(client, server);
            Assert.NotEmpty(client.NewSessionTickets);
        ...
        Assert.True(client2.HandshakeConfirmed, "Handshake (Verbindung 2) muss abschließen.");
        Assert.True(client2.ResumptionAccepted, "Client muss die PSK-Annahme erkennen.");
        Assert.True(server2.ResumptionAccepted, "Server muss den Binder akzeptiert haben.");
    }
}
