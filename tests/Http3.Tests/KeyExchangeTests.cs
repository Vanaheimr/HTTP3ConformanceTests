...
public class KeyExchangeTests
{
    ...
    public void X25519_TwoParties_DeriveSameSecret()
    {
        ...
        Assert.Equal(32, alice.PublicKey.Length);
        ...
        Assert.Equal(32, aliceView.Length);
    }
    ...
    public void X448_TwoParties_DeriveSameSecret()
    {
        ...
        Assert.Equal(56, alice.PublicKey.Length);
        byte[] aliceView = alice.DeriveSharedSecret(bob.PublicKey);
        byte[] bobView = bob.DeriveSharedSecret(alice.PublicKey);

        Assert.Equal(aliceView, bobView);
        Assert.Equal(56, aliceView.Length);
    }
    ...
    public void X448_MatchesRfc7748ScalarMultVector(string scalar, string inputU, string outputU)
    {
        ...
        Assert.Equal(outputU, Convert.ToHexString(secret).ToLowerInvariant());
    }
    ...
    public void X25519MlKem768_ClientServer_DeriveSameSecret()
    {
        ...
        Assert.Equal(1216, client.PublicKey.Length);

        (byte[] responseShare, byte[] serverSecret) = server.Encapsulate(client.PublicKey);
        Assert.Equal(1120, responseShare.Length);

        byte[] clientSecret = client.DeriveSharedSecret(responseShare);

        Assert.Equal(64, clientSecret.Length);
        Assert.Equal(serverSecret, clientSecret);
    }
    ...
    public void Factory_CreatesRightImplementation()
    {
        ...
        Assert.IsType<X25519KeyExchange>(x);
        Assert.IsType<X448KeyExchange>(x448);
        Assert.IsType<EcdheKeyExchange>(p256);
        Assert.Equal(32, x.PublicKey.Length);    // X25519: 32 Byte
        Assert.Equal(56, x448.PublicKey.Length); // X448: 56 Byte
        Assert.Equal(65, p256.PublicKey.Length); // P-256: unkomprimierter Punkt
    }
